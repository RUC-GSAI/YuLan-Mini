...
class LigerRopeFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, cos, sin, position_ids=None, unsqueeze_dim=2):
        """
...
def liger_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=2):
    """
    Applies Rotary Positional Embedding (RoPE) operation to query and key states.

    Args:
        q (torch.Tensor): The query tensor of shape (bsz, seq_len, n_q_head, head_dim).
        k (torch.Tensor): The key tensor of shape (bsz, seq_len, n_kv_head, head_dim).
        ...
        unsqueeze_dim (int, optional): The dimension to unsqueeze. Defaults to 2.
